...
func (r *SiteRepository) SyncUpstreamTasks(ctx context.Context, site Site, observedAt, overlapStart int64, snapshot dto.UpstreamTaskSnapshot, backfill bool) (int64, error) {
	...
	if len(items) > 0 {
		...
		for start := 0; start < len(ids); start += 500 {
			...
			var batch []SiteUpstreamTask
			if err := r.db.WithContext(ctx).Where("site_id = ? AND remote_id IN ?", site.ID, ids[start:end]).Find(&batch).Error; err != nil {
				return 0, err
			}
			existingRows = append(existingRows, batch...)
		}
	}
