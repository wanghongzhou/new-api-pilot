...
func (r *SiteRepository) SyncSystemTasks(ctx context.Context, site Site, at int64, snapshot dto.UpstreamSystemTaskSnapshot) (int64, error) {
	...
	var existingRows []SiteSystemTask
	if len(remoteIDs) > 0 {
		if err := r.db.WithContext(ctx).Where("site_id=? AND remote_id IN ?", site.ID, remoteIDs).Find(&existingRows).Error; err != nil {
			return written, err
		}
	}
	existing := make(map[int64]SiteSystemTask, len(existingRows))
	...
	for _, item := range items {
		...
		changedRows = append(changedRows, incoming)
		written++
	}
