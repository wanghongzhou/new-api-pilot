...
- 周期 ticker 使用固定 60 秒，但每站点加入 0～10 秒稳定抖动，抖动由 site_id 计算，重启后保持稳定；`performance_sync/topup_sync/redemption_sync/upstream_task_sync/model_meta_sync/plan_sync/pricing_group_sync/system_task_sync` 八个 durable task 统一复用该资源周期；
...
所有采集任务类型使用独立并发配额和同类型租约。`usage_backfill`、`usage_validation`、`account_rebuild`、`customer_rebuild` 各自拥有独立 limiter/metrics key；未配置专属值时沿用 `collector.backfill_concurrency` 作为各队列默认上限。站点首次授权产生的 `usage_backfill` 使用独立的 initial-backfill limiter，默认并发上限为 10，不占用普通 `collector.backfill_concurrency` 配额；首次回填 claim 一次领取该 run 的全部窗口，避免 24 窗口分片导致单个慢窗口阻塞后续批次，但实际请求仍严格限制为 10 个窗口并行，单窗口执行超时后回到 pending 重试；领取查询必须按 `CollectionPriorityInitialBackfill` 过滤，避免首次回填槽位耗尽时误领取普通回填。站点级 `usage_backfill` 运行时不得阻塞 `user_sync`、`channel_sync`、`site_probe`、`realtime_stat` 或 `resource_snapshot`；同一任务类型仍由 `active_key` 去重并受站点配置版本栅栏约束。
