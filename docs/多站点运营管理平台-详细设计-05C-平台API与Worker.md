...
SettingPatchRequest 是 key/value 数组，后端按白名单验证类型和范围。collector.probe_interval_seconds、collector.realtime_interval_seconds、collector.resource_interval_seconds 固定为 60 并标记 read_only，不能由 PUT 修改。敏感项 GET 只返回 configured、masked_value、decrypt_error，不返回 setting_value；PUT 中空字符串表示“不修改”，显式 clear=true 才清空。SettingGroup 同时返回 h15_slo_eligible/h15_slo_reason_codes，按 usage_delay_minutes<=5、usage_concurrency>=5 计算。h15_slo_reason_codes 是只允许 `SLO_USAGE_DELAY_TOO_HIGH`、`SLO_USAGE_CONCURRENCY_TOO_LOW` 的机器标志数组，不是完整 MessageRef；前端分别使用 `{value: usage_delay_minutes, threshold: 5}`、`{value: usage_concurrency, threshold: 5}` 构造 params 后本地化，禁止把原因码作为无参数 MessageRef 渲染。production PUT 产生 ineligible 配置时返回 SLO_CONFIG_FORBIDDEN 且整批不写。

GET 按 collector、export、rate、notification、system 固定顺序返回 23 个 platform_setting 以及只读虚拟项 `system.public_origin`；新增 `logs.retention_days` 位于 collector 分组，范围 1～3650。虚拟项来自运行环境、updated_at=null，PUT 必须拒绝。配置范围固定为：usage_delay_minutes 1～59，minute_retention_days/logs.retention_days 1～3650，六个队列 concurrency 1～100，manual_backfill_max_days 1～3660，export.file_ttl_hours 1～168，两个活跃导出上限 1～100 且单用户不得超过全局，max_file_bytes/min_free_disk_bytes 为正 int64。普通 int 只接受规范的正整数 JSON number，不接受字符串、小数、指数、符号或超过 `9007199254740991` 的值；两个字节阈值只接受规范的正 int64 十进制 JSON string，不接受 JSON number。两个 decimal 只接受 ASCII 固定点 JSON string，语法为整数位数字加可选的非空小数位，拒绝空白、符号、指数、非 ASCII 数字和零；小数位最多 10 位、总精度最多 30 位、规范化后整数位最多 20 位，并规范化整数前导零和小数尾零。CPU/内存/磁盘阈值、instance_stale 和任务重试预算不属于 platform_setting，继续由 alert-rules 或固定任务策略管理。

PUT 在进入事务前完成 items 数量、重复 key、白名单、read_only、JSON 表示、类型、单字段范围以及新 webhook 的 HTTPS/allowlist 校验。随后单事务按 setting_key 顺序 `SELECT ... FOR UPDATE` 锁定全部 22 行，构造最终状态并校验活跃导出上限、H+15 和钉钉完整配置，全部通过后才统一 UPDATE；任一错误必须零写入。事务采用原子 last-write-wins，不接收未文档化的 CAS/version 字段；只有实际变化的行更新 updated_at，值为 `max(now, previous_updated_at+1)`，未变化和敏感 keep 保持原时间。
...
`task_type` 仅允许 `site_probe|realtime_stat|resource_snapshot`，`status` 仅允许空、`success|failed`，offset>=0，limit=1..100。`FastTaskHistoryPage={items,offset,limit,total,has_more}`；item 字段为 site_id（IdString）、task_type、started_at、finished_at、status、duration_ms、error、request_id。Redis key 固定为 `new-api-pilot:fast-task:{site_id}:{task_type}`，每次 LPUSH/LTRIM 到 `FAST_TASK_HISTORY_COUNT` 并刷新 `FAST_TASK_HISTORY_RETENTION_SECONDS` TTL；状态筛选在服务端完成后再分页。参数非法返回 VALIDATION_ERROR，Redis 读取失败返回 INTERNAL_ERROR，不回退到伪造空列表。
...
| 422 | SLO_CONFIG_FORBIDDEN | production 配置会破坏 H+15 认证画像 |
...
| SLO eligibility | SLO_USAGE_DELAY_TOO_HIGH、SLO_USAGE_CONCURRENCY_TOO_LOW |
...
SettingSLOReasonCode = "SLO_USAGE_DELAY_TOO_HIGH" | "SLO_USAGE_CONCURRENCY_TOO_LOW"
...
- 周期 ticker 使用固定 60 秒，但每站点加入 0～10 秒稳定抖动，抖动由 site_id 计算，重启后保持稳定；
...
- collection_run 创建时保存 created_request_id 和当前 site_config_version；每次领取/重试生成新 attempt/CAS token 写 last_request_id，心跳、进度和结果提交都必须匹配该 token。collection 没有独立 lease 列，以 heartbeat_at 判定过期；单次领取最多处理 24 个窗口或 30 秒后让出；
- 单实例启动时先扫描 status=pending 且 windows_initialized_at=null 的窗口 run，并按 §32.9 的 scope→parent 锁序与 config/lifecycle CAS 继续物化，再无条件接管全部 collection/export running 任务（旧进程已不存在）；运行期间每 60 秒执行相同补偿。collection 以 heartbeat 超过 5 分钟判定过期，回收严格按 scope 对象→父 run→run-window 锁序，窗口型只按 attempt_count、非窗口只按父 retry_count 判断预算，均不重复增加领取时已计入的尝试。export 启动接管全部 running，运行期只接管 lease_expires_at 已过期的 running，删除半成品后按已计入的 attempt_count 决定 pending/failed。alert_delivery 始终保持 pending，fresh claim 和过期 lease takeover 由 claim_token/lease_expires_at 区分。这样创建/物化/执行任一断点都不会留下孤儿或永久占用，且旧 token 不能覆盖新 owner 或 fence 后终态；
- 优雅停机先停止创建新任务，再等待运行任务最多 30 秒；未完成任务保留 running，由下次启动无条件接管；
- 所有 ticker 使用 time.Ticker，不使用无限 time.Sleep 循环；
- 单实例进程内锁键必须从下表的“队列/锁 family”和实际 scope 派生，不能统一写成 task_type:site_id：站点任务为 `{family}:site:{site_id}`；用量窗口为 `usage:site:{site_id}:hour:{hour_ts}`；account/customer 重建一律为 `local_rebuild:customer:{customer_id}`，即使父 run.site_id 为空也能与同客户账户任务互斥。数据库行锁仍是最终正确性边界，进程锁只减少竞争。

任务类型是常量包、锁、重试和 API 筛选的共同权威枚举：

| task_type | target | 队列/锁 family | 窗口/游标 | 默认重试 |
|---|---|---|---|---|
| site_probe | site | probe | 否/否 | 等下一周期 |
| realtime_stat | site | realtime | 否/否 | 等下一周期 |
| resource_snapshot | site | resource | 分钟/否 | 等下一周期 |
| user_sync | site | metadata | 否/否 | 等下一小时 |
| channel_sync | site | metadata | 否/否 | 等下一小时 |
| usage_hour | site | usage | 小时/共享 usage 游标推进 | 小时采集策略 |
| usage_backfill | site | usage | 小时/共享 usage 游标推进 | 历史回填策略 |
| usage_validation | site | usage | 小时/仅回退或按连续 complete 推进共享 usage 游标 | 校验策略 |
| account_rebuild | account | local_rebuild:customer_id | 小时 run-window/无游标 | 历史回填策略 |
| customer_rebuild | customer | local_rebuild:customer_id | 按 site+小时 run-window/无游标 | 历史回填策略 |
...
每次 60 秒探活/资源/账户同步以及每次完整 channel_sync 快照提交后评估对应规则。连续次数由 alert_event.consecutive_count 保存，进程重启不丢失。阈值从单站覆盖回落全局默认；Warning 和 Critical 同时满足时只保留 Critical，已有 Warning firing 先 resolved，再创建 Critical firing。恢复条件使用阈值本身，不额外引入迟滞区间。
...
1. 02:00 次日业务校验；
2. 03:00 重建前一天资源 daily 并置 final；
3. 03:20 扫描并幂等补建目标分钟数据缺失的 hourly/daily，确认全部已固化 sample/expected/data_status；
4. 03:30 删除 retention_days 以前的实例/站点分钟行；
5. 03:40 清理过期导出文件；
6. 04:00 清理 90 天前永久保留 collection_run 的 error_message 详细文本，仅保留稳定 code/params、状态、范围和计数；

7. 清理 30 天前成功的自动 user_sync/channel_sync 诊断 run；失败、手动、窗口型、本地重建 run 永久保留。probe/realtime/resource 成功周期本就不落 run。
...
所有采集任务类型使用独立并发配额和同类型租约。`usage_backfill`、`usage_validation`、`account_rebuild`、`customer_rebuild` 各自拥有独立 limiter/metrics key；未配置专属值时沿用 `collector.backfill_concurrency` 作为各队列默认上限。站点级 `usage_backfill` 运行时不得阻塞 `user_sync`、`channel_sync`、`site_probe`、`realtime_stat` 或 `resource_snapshot`；同一任务类型仍由 `active_key` 去重并受站点配置版本栅栏约束。
# P0-B 日志 API 与 Worker

Worker 新增 `log_sync` 任务类型，使用 site/config_version fence、有限重叠时间窗口和稳定去重；任务失败遵循现有 retry/lease 规则。API 新增 `GET /api/logs` 与 `GET /api/sites/:id/logs`，筛选字段与 `/api/log/` 对齐但采用平台 snake_case DTO，权限沿用站点范围授权。新增日志导出 scope `logs`，复用 `export_job` 生命周期和文件 TTL。
...
新增任务 `performance_sync`，调度周期复用固定 60 秒资源周期，独立 active_key/lease/concurrency/retry；每轮采集最近 24 小时官方 buckets 并按唯一键幂等覆盖。`performance.retention_days` 默认 90、范围 1～3650，成功提交后删除 cutoff 前历史；清理失败不得撤销已提交快照，但进入可观测错误与下轮重试。
