...
- complete 无记录才显示 0；
- 当前日注明“截至最近完整小时”；
- 统计页面与源站后台的“今日/实时”页面不得直接横向比较：平台统计只纳入已经完成结算并通过 `/api/data` 与 `/api/data/flow` 守恒校验的自然小时；小时粒度默认范围的 `end` 是北京时间当前小时起点，日/月/年粒度仍分别按次日、下月、明年起点对齐。源站当前尚未结束小时会继续增长，因此平台汇总可能暂时较小。页面必须同时显示 `as_of`，并明确提示该口径，避免把采集延迟误判为数据丢失；对账必须使用相同的左闭右开时间范围。
- 月、年活跃用户不能由 daily 直接相加；
- `summary.active_users` 表示整个查询范围内 `(site_id,remote_user_id)` 的精确去重数，任何粒度都不得把小时或日期桶的 `active_users` 相加。小时查询按北京时间自然日拆分：首尾非完整自然日只读取对应边界小时的 `usage_fact_hourly` 身份，中间完整自然日只读取 `usage_fact_daily` 身份，合并后再做一次范围去重；完整性仍只允许 `collection_window.status=complete` 的身份进入结果。这样 31 天查询不扫描整个范围的小时事实，同时保持与事实重建一致的精确语义。
- 客户、模型、通道的 `hour/day` breakdown 活跃用户直接读取对应预聚合表；客户身份按账户唯一归属，可由同一桶的客户/站点行无重叠汇总出 trend、site breakdown，并在单桶查询中得到精确 summary。只有跨多个桶的范围 summary，或模型/通道跨维度去重，才读取身份事实；未指定客户/账户筛选时不得把全量实体 ID 展开为巨大 `IN` 条件。
- 全局小时趋势的每个 `(site_id,bucket_start)` 在响应中只能出现一次。后端以 `site_stat_hourly` 的站点桶作为唯一逐站输入，同一批站点桶同时生成全局 trend、全局维度值和 `site_breakdown`；禁止用 `dimension` 与 `site` 两类内部行重复返回相同站点/小时数据，也禁止为了组装响应再次读取同一范围。
- 不少于 7 天的全局小时查询允许使用 30 秒短缓存并合并同参数并发请求；缓存只保存成功响应，所有无序筛选集合排序归一化后才生成 key，导出不使用该缓存。缓存 TTL 使用进程墙钟，不受业务测试时钟或固定验收时钟影响；缓存 miss 仍必须满足本节的汇总表与精确身份查询约束。
- 跨站金额由前端逐站换算；
...
站点管理列表承载每个站点的三层运营概览：站点身份与管理/健康/授权状态、实时实例与资源状态，以及与当前 new-api 数据看板同名同口径的“总数、总额度、总 Token 数、平均 RPM、平均 TPM、成功率、平均延迟、吞吐量”。前五项读取滚动最近 24 小时的最新本地采集汇总；列表不展示实时 RPM/TPM，避免与 24 小时平均口径混用。后三项读取最近一次成功采集的 24 小时性能 summary，并复用 new-api `PerformanceHealthPanel` 的逐模型简单平均规则；不加载趋势或完整统计明细。性能缓存不可用时显示不可用状态。
...
3. 近 30 天趋势
   ...
   - quota/金额；
   - Token。
   - 只读取本地预聚合的站点/全局小时桶；不得为趋势或汇总回扫 30 天 `usage_fact_hourly`，活跃用户范围去重遵循 §18.2 的“边界小时 + 中间 daily 身份”规则。
