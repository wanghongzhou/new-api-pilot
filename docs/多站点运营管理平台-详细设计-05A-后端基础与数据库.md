...
| Migration | `migrations/0001_initial_schema.sql` 为完整基线，`0002_alert_threshold_precision.sql` 将告警规则及事件阈值收敛为 2 位小数；`model/migration.go` 负责 checksum、断点恢复和执行 |
...
- `0001_initial_schema.sql` 是不可变完整基线；后续结构变化通过更高版本前向迁移追加，当前 `0002_alert_threshold_precision.sql` 将 alert_rule/alert_event 的 threshold_value 从 DECIMAL(30,10) 收敛为 DECIMAL(30,2)。GORM AutoMigrate 仅允许测试环境使用，不作为生产迁移机制；
