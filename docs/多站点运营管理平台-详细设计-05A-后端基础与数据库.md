...
| Migration | migrations/0001_initial_schema.sql、migrations/0002_*.sql，model/migration.go 负责 checksum 和执行 |
...
- TRUSTED_PROXIES 显式列出可信反向代理 CIDR，Gin 不信任列表外的 X-Forwarded-*；
- UPSTREAM_ALLOWED_HOST_SUFFIXES/UPSTREAM_ALLOWED_CIDRS 至少配置一项；UPSTREAM_CA_FILE 可选加载企业 CA，禁止 InsecureSkipVerify；
...
- 生产迁移只使用平台仓库内的版本化 SQL，按 `0001_initial_schema.sql`、`0002_xxx.sql` 顺序执行；GORM AutoMigrate 仅允许测试环境使用，不作为生产迁移机制；
...
- MySQL DDL 会隐式提交，因此不得宣称整个 migration 天然原子。非 DDL 语句按单条语句连同 checkpoint 放在同一事务中；全部语句完成后才原子写 schema_migration 并删除 progress。0002～0005 的每条 DDL 都必须有明确 postcondition，缺失时拒绝执行；
...
VALUES
  ...
  ('rate.fallback_quota_per_unit', '', 'decimal', 0, UNIX_TIMESTAMP()),
  ('rate.fallback_usd_exchange_rate', '', 'decimal', 0, UNIX_TIMESTAMP()),
