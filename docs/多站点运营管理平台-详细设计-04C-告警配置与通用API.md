...
| 渠道可用率 | <= 0.90 | <= 0.80 | Warning 连续 3 个 complete 小时快照，Critical 立即 |
...
- id、rule_key、name；
- enabled、level、metric、compare_operator、threshold_value、for_times；其中规则阈值只接受非负十进制定点字符串，最多 2 位小数；
- scope_type、scope_id；
...
规则列表按业务对象提供稳定分类 `site/collection/instance/account/channel`。分类来自内建规则契约而不是 metric 字符串猜测；渠道聚合规则即使事件 target_type 为 site，规则分类仍为 channel。规则列表必须服务端分页，并支持 category、level、enabled、inherited 过滤。`inherited` 仅描述站点有效规则是否直接继承全局行；全局规则恒为 false。排序只开放 category、rule_key、level、metric、enabled、updated_at；连续次数是触发配置而不是定位维度，不提供排序，threshold_value 因百分比、秒、次数、额度和比例单位不同，也不提供跨规则排序。默认顺序为 category → rule_key → level（Critical、Warning、Info）→ id，任意显式排序仍以 rule_key、level、id 作为稳定尾排序。
...
- current_value、threshold_value、message_code、message_params、message；事件 current_value 保留指标原始精度，threshold_value 是规则阈值快照，最多 2 位小数；
...
| 分组 | 配置 |
|---|---|
| 采集 | 站点状态检查、资源、RPM/TPM 周期可在 1～60 分钟内热更新；小时采集延迟可配置 |
| 并发 | 各任务队列并发上限 |
...
配置值校验范围：站点状态检查、实时统计和资源采集周期 1～60 分钟，API 以 60～3600 的整分钟秒数保存；小时采集延迟 1～59 分钟；队列并发 1～100；手动补采上限 1～3660 天；分钟留存 1～3650 天；导出有效期 1～168 小时、活跃任务上限 1～100、文件和磁盘阈值必须为正数；兜底汇率为空或正数。任何一项非法时整批 PUT 不落库。CPU/内存/磁盘阈值和连续次数只通过 alert-rules API 修改，不写 platform_setting。
...
API 进程对运行时使用的快速任务历史、上游地址白名单、CIDR、超时、限流和单 origin 并发维护不可变内存快照。写入成功的实例在事务提交后立即发布新快照；所有其他实例每秒从 `platform_setting` 读取并校验完整白名单，数据库版本较新时原子替换，最迟 2 秒内生效。轮询不得依赖 Redis 或新增外部组件。读取失败或契约校验失败时保留最后一个有效快照。限流参数变化必须原地重配现有 governor，保留已排队请求、在途计数、下一次允许时间和 429 cooldown；降低并发时不取消已在途请求，但在途数降到新上限前不得发放新请求。
