...
func TestA101SiteTaskCatalogMatchesBackendTaskContract(t *testing.T) {
	...
	want := map[string]struct {
		...
		constant.TaskTypePerformanceSync:  {"durable", "operational_interval"},
		constant.TaskTypeTopupSync:        {"durable", "operational_interval"},
		constant.TaskTypeRedemptionSync:   {"durable", "operational_interval"},
		constant.TaskTypeUpstreamTaskSync: {"durable", "operational_interval"},
		constant.TaskTypeModelMetaSync:    {"durable", "catalog_interval"},
		constant.TaskTypePlanSync:         {"durable", "catalog_interval"},
		constant.TaskTypePricingSync:      {"durable", "catalog_interval"},
		constant.TaskTypeSystemTaskSync:   {"durable", "catalog_interval"},
