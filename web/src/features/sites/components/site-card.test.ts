...
test('uses one non-rounding completeness formatter in card and table views', async () => {
  const [cardSource, pageSource] = await Promise.all([
    readFile(new URL('./site-card.tsx', import.meta.url), 'utf8'),
    readFile(new URL('./sites-page.tsx', import.meta.url), 'utf8'),
  ])

  expect(cardSource).toContain('formatCompletenessPercent(value)')
  expect(pageSource).toContain('formatCompletenessPercent(value)')
  expect(cardSource).not.toContain('percent.toFixed(0)')
...
test('places total count with average RPM and TPM on the lower row', async () => {
  ...
  expect(lowerRow).toContain("t('site.dashboard.last24HoursCount')")
...
test('keeps the approved card content in the grouped desktop list', async () => {
  ...
  expect(source).toContain("label={t('site.dashboard.last24HoursQuota')}")
  expect(source).toContain("label={t('site.dashboard.last24HoursTokens')}")
  expect(source).toContain("label={t('site.dashboard.last24HoursCount')}")
  expect(source).toContain('formatAverageRate(today.avg_rpm)')
  expect(source).toContain('formatAverageTpm(today.avg_tpm)')
